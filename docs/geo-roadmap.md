...
Last updated: 2026-04-28 (completed a fresh broad reconciliation across the generic service-business strategy cluster, then chose the bounded maintenance path the roadmap had already flagged: a distinct second refresh on `/ai-automation-cost-small-business`. The page now does more than list price ranges. It explains what each budget band should actually buy first, routes those bands directly into the first-project guide, consultant pricing, lead-follow-up cost, voice-agent setup cost, and proof hub, and tightens the proof framing around named production outcomes from the Paris Cafe, WheelsFeels, and Instagram case studies.)
...
This run completed the fresh broad reconciliation the roadmap called for and then used that reconciliation to justify a bounded refresh on `/ai-automation-cost-small-business` instead of forcing another adjacent generic service-business child. The page stays distinct from `/ai-automation-consultant-pricing-small-business` and `/what-to-automate-first-for-service-businesses` by answering the narrower budget-intent question: what should each automation budget band actually buy first, and when should an owner stay with one bounded workflow versus move to a broader system.

The next run should:

1. **Keep manual GSC follow-through moving** on `/ai-automation-cost-small-business` and the other refreshed priority URLs instead of treating publish alone as the finish line.
2. **Do another fresh broad reconciliation before naming a net-new generic service-business page** so the next ship comes from a real gap across planning docs, live routes, tracked prompts, and priority URLs rather than inertia.
3. **If no stronger net-new page survives that reconciliation, do a bounded refresh on `/ai-automation-roi-guide`** so the ROI hub routes more explicitly into the newer cost, consultant-pricing, support, proof, and first-project pages that now form the core commercial decision cluster.

| Priority | Candidate | Why it still looks viable | Required re-check before shipping |
|---|---|---|---|
| 1 | Manual GSC follow-through on refreshed priority URLs | Several of the highest-value pages are still in discovered/not-indexed or not-yet-inspected state, so distribution and crawl feedback still matter as much as shipping. | Re-open `docs/geo-priority-urls.md`, inspect the newest refreshed pages in GSC, and record whether the crawl/index state moved after the latest deploys. |
| 2 | Fresh broad reconciliation before another net-new generic service-business page | The generic strategy cluster is materially cleaner after the first-project ship plus the cost-hub refresh, so the safest next content move should still come from a repo-wide pass rather than another assumed same-cluster child. | Reconcile `GEO-PLAN.md`, `docs/programmatic-seo-pattern-matrix.md`, `docs/geo-priority-urls.md`, current routes/pages, and `scripts/geo-prompts.json` again before naming the next net-new candidate. |
| 3 | `/ai-automation-roi-guide` bounded refresh | Now that the cost hub routes more clearly into proof, pricing, and first-project selection, the ROI hub is the most natural adjacent maintenance target in the same commercial decision cluster. | Re-check the live ROI copy, the latest cost/pricing/support pages, and whether the strongest proof and first-project links are already visible near the top of the page before choosing the exact refresh scope. |
